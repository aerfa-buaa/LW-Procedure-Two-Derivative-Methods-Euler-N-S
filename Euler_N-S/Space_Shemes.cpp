...
#pragma omp critical
	...
	if (Viscous_flag == 1 && DT > mindt_vis)
		DT = mindt_vis;

	if (Viscous_flag == 1 && DT > mindt_k)
		DT = mindt_k;

	// cout << "Step = " << ExtIter << " mindt_k = " << mindt_k << endl;
	// cout << "Step = " << ExtIter << " mindt_vis = " << mindt_vis << endl;
	// cout << "Step = " << ExtIter << " DT = " << DT << endl;

	if ((TimeNow + DT) > StopTime)
	{
		DT = StopTime - TimeNow;
		T_flag = 10.;
	}
...
void CADSolver::Splitting_char(void)
{

	mydouble ss1 = 0., ss2 = 0., pp1 = 0., pp2 = 0.;

	Splitting_char_x(1.0, 0.);
...
#pragma omp parallel for private(ss1, ss2, pp1, pp2)
	for (long jCell = nb; jCell < nCellYo_L; jCell++)
	{
		for (long iCell = nb; iCell < nCell_L; iCell++)
		{
			...
			{
				ss1 = (D_fxz[iCell][jCell][nn] - D_fxz[iCell - 1][jCell][nn]) / dx;
				// ss2 = (D_fyz[iCell][jCell][nn] - D_fyz[iCell][jCell - 1][nn]) / dy;
				ss2 = 0.;
				D_du[iCell][jCell][nn] = ss1 + ss2;

				pp1 = (D_fxz_up1[iCell][jCell][nn] - D_fxz_up1[iCell - 1][jCell][nn]) / dx;
				// pp2 = (D_fyz_up1[iCell][jCell][nn] - D_fyz_up1[iCell][jCell - 1][nn]) / dy;
				pp2 = 0.;
...
#pragma omp parallel for
	for (long jCell = nb - 4; jCell < nCellYo_L + 4; jCell++)
	{
		...
		mydouble qz[15] = {0}, qf[10] = {0}, wz[4][4] = {0}, wf[4][4] = {0};
		long n_w = 3, mm;
...
#pragma omp parallel for
	for (long jCell = nb - 4; jCell < nCellYo_L + 4; jCell++)
	{
		...
		// mydouble lamda[4][4] = {0}, lamda_z[4][4] = {0}, lamda_f[4][4] = {0};
		mydouble qz[10] = {0}, qf[10] = {0}, wz[4][4] = {0}, wf[4][4] = {0};
		long n_w = 3, mm;
		for (long iCell = nb - 4; iCell < nCell_L + 4; iCell++)
		{
...
void CADSolver::Splitting_char_x(mydouble nx, mydouble ny)
{
	// Book: Riemann Solversand Numerical Methods for Fluid Dynamics, Third Edition  Toro PP.108 275 355
	// Book: I Do Like CFD   pp.69

#pragma omp parallel for
	for (long jCell = nb - 1; jCell < nCellYo_L + 1; jCell++)
	{
		mydouble fxz[5], fxz_up1[5], fyz[5], RM[4][4], LM[4][4];
		mydouble a1, a2, u1, u2, v1, v2, c1, c2, H1, H2, r1, r2, r0, u0, v0, H0, c0, uv, uvxy, lmax, Zc, rt, a3, a4, phi;
		mydouble lamda[4][4] = {0}, lamda_z[4][4] = {0}, lamda_f[4][4] = {0};
		mydouble qz[15] = {0}, qf[10] = {0}, wz[4][4] = {0}, wf[4][4] = {0};
		long n_w = 5, mm;
		for (long iCell = nb - 1; iCell < nCell_L + 1; iCell++)
		{
			...
			rt = (a1 + a2) / 2.0;
			a1 = Gamma_1;
			a2 = 1.0 / (sqrt(2.0) * c0 * rt);
			a3 = rt / c0 / sqrt(2.0);
			phi = Gamma_1 * uv / 2.0;
			a4 = (phi + c0 * c0) / Gamma_1;

			uvxy = nx * u0 + ny * v0;
			Zc = Gamma_1 / (c0 * c0);
			lamda[0][0] = uvxy, lamda[1][1] = uvxy, lamda[2][2] = uvxy + c0, lamda[3][3] = uvxy - c0;
			// !  特征矩阵S  (A的右特征矩阵)λ1 = u − c, λ2 =  u  ,λ3 =  u+ c, λ4 = u   .
			...
			RM[3][0] = phi / a1;
			RM[3][1] = rt * (ny * u0 - nx * v0);
			RM[3][2] = a3 * (a4 + c0 * uvxy);
			RM[3][3] = a3 * (a4 - c0 * uvxy);

			LM[0][0] = 1.0 - phi / (c0 * c0);
			LM[0][1] = a1 * u0 / (c0 * c0);
			LM[0][2] = a1 * v0 / (c0 * c0);
			LM[0][3] = -a1 / (c0 * c0);

			LM[1][0] = (nx * v0 - ny * u0) / rt;
			LM[1][1] = ny / rt;
			LM[1][2] = -nx / rt;
			LM[1][3] = 0.0;

			LM[2][0] = a2 * (phi - c0 * uvxy);
			LM[2][1] = a2 * (nx * c0 - a1 * u0);
			LM[2][2] = a2 * (ny * c0 - a1 * v0);
			LM[2][3] = a1 * a2;

			LM[3][0] = a2 * (phi + c0 * uvxy);
			LM[3][1] = -a2 * (nx * c0 + a1 * u0);
			LM[3][2] = -a2 * (ny * c0 + a1 * v0);
			LM[3][3] = a1 * a2;
			...
			for (long nn = 0; nn < 4; nn++)
			{
				lamda_z[nn][nn] = (lamda[nn][nn] + lmax) * 0.5;
				lamda_f[nn][nn] = (lamda[nn][nn] - lmax) * 0.5;
			}
			...
			for (long i = 0; i < 4; i++)
			{
				for (long j = 0; j < 4; j++)
				{
					...
					for (long k = 0; k < 4; k++)
					{
						wz[i][j] += lamda_z[i][k] * LM[k][j];
						wf[i][j] += lamda_f[i][k] * LM[k][j];
					}
				}
			}
			...
			for (long i = 0; i < 4; i++)
			{
				for (long m = -n_w; m < n_w + 1; m++)
				{
					...
					for (long k = 0; k < 4; k++)
					{
						qz[mm] += wz[i][k] * D_un[iCell + m][jCell][k];
						qf[mm] += wf[i][k] * D_un[iCell + m][jCell][k];
					}
				}
			...
			for (long i = 0; i < 4; i++)
			{
				D_fxz[iCell][jCell][i] = 0., D_fxz_up1[iCell][jCell][i] = 0.;
				for (long k = 0; k < 4; k++)
				{
					D_fxz[iCell][jCell][i] += RM[i][k] * fxz[k];
					D_fxz_up1[iCell][jCell][i] += RM[i][k] * fxz_up1[k];
				}
			}
		}
	}
...
void CADSolver::Splitting_char_y(mydouble nx, mydouble ny)
{
	// Book: Riemann Solversand Numerical Methods for Fluid Dynamics, Third Edition  Toro PP.108 275 355
	// Book: I Do Like CFD   pp.69
...
#pragma omp parallel for
	...
	{
		mydouble fxz[5], fyz[5], fyz_up1[5], RM[4][4], LM[4][4];
		mydouble a1, a2, u1, u2, v1, v2, c1, c2, H1, H2, r1, r2, r0, u0, v0, H0, c0, uv, uvxy, lmax, Zc;
		mydouble lamda[4][4] = {0}, lamda_z[4][4] = {0}, lamda_f[4][4] = {0};
		mydouble qz[10] = {0}, qf[10] = {0}, wz[4][4] = {0}, wf[4][4] = {0};
		long n_w = 4, mm;
		for (long iCell = nb - 1; iCell < nCell_L + 1; iCell++)
		{
			...
			uvxy = nx * u0 + ny * v0;
			Zc = Gamma_1 / (c0 * c0);
			lamda[0][0] = uvxy - c0, lamda[1][1] = uvxy, lamda[2][2] = uvxy + c0, lamda[3][3] = uvxy;
			// !特征矩阵S  (A的右特征矩阵) λ1 = u − c, λ2 =  u  ,  λ3 =  u+ c,  λ4 = u.
			// A=RM.S.LM
			// 将下面4行的数据，每行数据用,隔开了，数据是4行4列。 将4行4列数据赋值到二维RM中  ,
			RM[0][0] = 1.0, RM[0][1] = 1.0, RM[0][2] = 1.0, RM[0][3] = 0.0;
			RM[1][0] = -c0 * nx + u0, RM[1][1] = u0, RM[1][2] = c0 * nx + u0, RM[1][3] = -ny;
			RM[2][0] = -c0 * ny + v0, RM[2][1] = v0, RM[2][2] = c0 * ny + v0, RM[2][3] = nx;
			RM[3][0] = H0 - c0 * uvxy, RM[3][1] = uv / 2., RM[3][2] = H0 + c0 * uvxy, RM[3][3] = -ny * u0 + nx * v0;
			//  1, 1, 1, 0,
			//  -c0 nx + u0, u0, c0 nx + u0, -ny,
			//  -c0 ny + v0, v0, c0 ny + v0, nx,
			//  H0 - c0 (nx u0 + ny v0), uv/2,  H0 + c0 nx u0 + c0 ny v0, -ny u0 + nx v0
			LM[0][0] = 1. / 4. * ((2. * uvxy) / c0 + uv * Zc);
			LM[0][1] = -((nx + c0 * u0 * Zc) / (2. * c0));
			LM[0][2] = -((ny + c0 * v0 * Zc) / (2. * c0));
			LM[0][3] = Zc / 2.;
			...
			LM[3][0] = ny * u0 - nx * v0, LM[3][1] = -ny, LM[3][2] = nx, LM[3][3] = 0.;
			// {1/4 ((2 uvxy)/c0 + uv Zc), -((nx + c0 u0 Zc)/(2 c0)), -(( ny + c0 v0 Zc)/(2 c0)), Zc/2},
			//  {1 - (uv Zc)/2, u0 Zc, v0 Zc, -Zc},
			//  {1/4 (-((2 uvxy)/c0) + uv Zc), (nx - c0 u0 Zc)/(2 c0), ( ny - c0 v0 Zc)/(2 c0), Zc/2},
			//  {ny u0 - nx v0, -ny, nx, 0}
			lmax = fabs(uvxy) + c0;
			for (long nn = 0; nn < 4; nn++)
			...
			for (long i = 0; i < 4; i++)
			{
				for (long m = -n_w; m < n_w + 1; m++)
				{
					...
					for (long k = 0; k < 4; k++)
					{
						qz[mm] += wz[i][k] * D_un[iCell][jCell + m][k];
						qf[mm] += wf[i][k] * D_un[iCell][jCell + m][k];
					}
				}
			...
			for (long i = 0; i < 4; i++)
			{
				D_fyz[iCell][jCell][i] = 0., D_fyz_up1[iCell][jCell][i] = 0.;
				for (long k = 0; k < 4; k++)
				{
					D_fyz[iCell][jCell][i] += RM[i][k] * fyz[k];
					D_fyz_up1[iCell][jCell][i] += RM[i][k] * fyz_up1[k];
				}
			}
		}
	}
...
//___________________ Viscous_Splitting_LW __________________
...
#pragma omp parallel for // num_threads(NumberThreads)
	for (long jCell = nb - 3; jCell < nCellYo_L + 4; jCell++)
	{
		...
		for (long iCell = nb - 3; iCell < nCell_L + 4; iCell++)
		{
			...
			D_gx[iCell][jCell][1] = tau11t;
			D_gx[iCell][jCell][2] = tau12t;
			D_gx[iCell][jCell][3] = ka * sTx + ut_0 * tau11_0 + tau11t * u_0 + vt_0 * tau12_0 + tau12t * v_0;

			D_gy[iCell][jCell][1] = tau12t;
			D_gy[iCell][jCell][2] = tau22t;
			D_gy[iCell][jCell][3] = ka * sTy + ut_0 * tau12_0 + tau12t * u_0 + vt_0 * tau22_0 + tau22t * v_0;
		}
	}
	// #pragma omp barrier
	Viscous_Schemes_LW();
